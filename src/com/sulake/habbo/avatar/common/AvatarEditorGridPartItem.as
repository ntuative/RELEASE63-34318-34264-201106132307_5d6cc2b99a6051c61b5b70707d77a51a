package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1511:IWindow;
      
      private var var_887:IFigurePartSet;
      ...
      private var var_718:Rectangle;
      
      private var var_2103:int = 0;
      
      private var var_494:IAvatarRenderManager;
      
      private var var_1121:Boolean;
      
      private var var_2102:BitmapData;
      
      private const const_2318:int = 13421772;
      
      private const const_2319:int = 16777215;
      
      private const const_384:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_887 = param3;
         this._window = param1;
         this.var_1511 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2103 = Math.max(this.var_2103,_loc6_.colorLayerIndex);
            }
         }
         this.var_494 = this.var_76.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_737 != null && !this.var_2104)
            {
               _loc3_ = this.var_737;
            }
            else
            ...
            if(this.var_887)
            {
               switch(this.var_887.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_791;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1511.color = this.const_2318;
         }
         else
         {
            this.var_1511.color = this.const_2319;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_494.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_494.isFigureReady(_loc1_))
         {
            this.var_494.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_384[_loc6_] + "_" + FigureData.const_149;
               _loc4_ = this.var_494.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_384.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_384[_loc6_] + "_" + FigureData.const_149;
                  _loc4_ = this.var_494.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_718 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2102)
               {
                  ...
                  this.var_2102 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_718.width,this.var_718.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_718.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_718.y;
               if(this.var_2104 && _loc2_.colorLayerIndex > 0)
               {
