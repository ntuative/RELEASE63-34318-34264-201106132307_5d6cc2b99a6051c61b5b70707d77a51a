package com.sulake.habbo.sound
{
   ...
   public class HabboSoundManagerFlash10 extends Component implements IHabboSoundManager, IUpdateReceiver
   {
      ...
      private static const const_1605:int = 60;
      
      private static const const_1606:int = 25165823;
      
      private static const const_1607:int = 16777215;
      ...
      private var var_859:TraxSequencer;
      ...
      public function HabboSoundManagerFlash10(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null, param4:Boolean = true)
      {
         ...
         events.addEventListener(TraxSongLoadEvent.const_878,this.onTraxLoadComplete);
      ...
      public function playSound(param1:String) : void
      {
         ...
         var _loc4_:* = null;
         var _loc5_:* = null;
         var _loc2_:HabboSound = this.var_1059.getValue(param1);
         if(_loc2_ == null)
         {
            _loc3_ = "";
            switch(param1)
            {
               case HabboSoundTypesEnum.const_1209:
                  ...
                  break;
               case HabboSoundTypesEnum.const_1723:
               ...
               case HabboSoundTypesEnum.const_1210:
                  ...
                  break;
               case HabboSoundTypesEnum.const_1881:
            ...
            this.var_1059.add(param1,_loc2_);
         }
         _loc2_.volume = this.var_1021;
         _loc2_.play();
      }
      ...
      private function loadSample(param1:int) : void
      {
         ...
         var _loc2_:String = this.var_169.getKey("flash.dynamic.download.url","furniture/");
         _loc2_ += this.var_169.getKey("flash.dynamic.download.samples.template","mp3/sound_machine_sample_%typeid%.mp3");
         ...
         _loc4_.load(_loc3_);
         this.var_691.add(_loc4_,param1);
      }
      ...
      private function addTraxSongForDownload(param1:int, param2:String) : IHabboSound
      {
         ...
         if(!_loc3_.ready)
         {
            this.var_1298.add(param1,_loc3_);
         }
         return _loc3_;
      }
      ...
      private function createTraxInstance(param1:int, param2:String, param3:Boolean = true) : TraxSequencer
      {
         ...
         var _loc5_:TraxSequencer = new TraxSequencer(param1,_loc4_,this.var_287,events);
         _loc5_.volume = this.var_1021;
      ...
      private function validateSampleAvailability(param1:TraxSequencer, param2:Boolean) : void
      {
         ...
         {
            if(this.var_287.getValue(int(_loc4_[_loc6_])) == null)
            {
               if(param2)
               {
      ...
      public function notifyPlayedSong(param1:String, param2:String) : void
      {
         ...
         this.var_1060.addSongPlayingNotification(param1,param2);
      }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         if(param2 != null)
         {
            ...
            if(this.var_169.getBoolean("trax.player.sample.memory.purge.enabled",false))
            {
               this.var_2775 = true;
            }
         }
      ...
      private function onNotificationsReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_1060 = IHabboNotifications(param2);
      }
      ...
      {
         if(this.var_169 == null || this._connection == null || this._roomEngine == null || this.var_236 != null)
         {
            return;
         }
      ...
      {
         events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_1989,this.var_858));
         this.var_858 = -1;
         this.var_859 = null;
      }
      ...
      private function storeVolumeSetting() : void
      {
         ...
         {
            this._connection.send(new SetSoundSettingsComposer(int(this.var_1021 * 100)));
         }
      }
      ...
      private function updateVolumeSetting(param1:Number) : void
      {
         ...
         this.var_236.updateVolume(this.var_1021);
      }
      ...
      private function processLoadedSample(param1:Sound) : void
      {
         ...
         {
            _loc2_ = this.var_691.remove(param1);
            if(this.var_287.getValue(_loc2_) == null)
            {
               this.var_1607.clear();
               _loc3_ = param1.length;
               param1.extract(this.var_1607,int(_loc3_ * 44.1));
               _loc4_ = new TraxSample(this.var_1607,_loc2_,TraxSample.const_1860,TraxSample.const_1851);
               this.var_287.add(_loc2_,_loc4_);
            }
         }
      }
      ...
      private function processLoadedSamples() : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         if(this.var_1299.length > 0)
         {
            _loc1_ = getTimer();
            _loc2_ = _loc1_;
            while(_loc2_ - _loc1_ < const_1605 && this.var_1299.length > 0)
            {
               _loc3_ = this.var_1299.splice(0,1)[0];
               this.processLoadedSample(_loc3_);
               _loc2_ = getTimer();
            }
            if(this.var_691.length == 0 && this.var_858 != -1)
            {
               this.var_859.ready = true;
               _loc4_ = this.var_858;
               this.var_858 = -1;
               this.var_859 = null;
               events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_878,_loc4_));
               if(this.var_2775)
               {
      ...
      private function loadNextSong() : void
      {
         var _loc1_:int = 0;
         var _loc2_:* = null;
         if(this.var_859 == null && this.var_1298.length > 0)
         {
            _loc1_ = this.var_1298.getKey(0);
            _loc2_ = this.var_1298.remove(_loc1_);
            if(_loc2_ != null && !_loc2_.disposed)
            {
               this.validateSampleAvailability(_loc2_,true);
               if(_loc2_.ready)
               {
                  events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_878,_loc1_));
               }
               else
               {
                  this.var_859 = _loc2_;
      ...
      private function processSampleMemoryUsage() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:* = [];
         var _loc3_:Array = this.var_236.samplesIdsInUse;
         var _loc4_:int = 0;
         while(_loc4_ < this.var_287.length)
         {
            _loc5_ = this.var_287.getKey(_loc4_);
            _loc6_ = this.var_287.getWithIndex(_loc4_);
         ...
         if(_loc1_ > const_1606)
         {
            ...
            _loc8_ = 0;
            _loc9_ = 0;
            while(_loc8_ < _loc1_ - const_1607 && _loc9_ < _loc2_.length)
            {
            ...
            if(_loc7_.length > 0)
            {
               for each(_loc11_ in _loc7_)
               {
                  ...
                  _loc12_ = this.var_287.getValue(_loc11_) as TraxSample;
                  _loc12_.dispose();
                  this.var_287.remove(_loc11_);
               }
               this.var_236.samplesUnloaded(_loc7_);
            }
         }
      }
