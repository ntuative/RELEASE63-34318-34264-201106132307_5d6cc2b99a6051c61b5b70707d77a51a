package com.sulake.habbo.ui.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_543:IItemGridWindow;
      
      private var _view:DimmerView;
      
      private var var_1811:XML;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_543.numGridItems)
         {
            return;
         }
         this.select(this.var_543.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_543 == null)
         {
            return;
         }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_233.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_543.destroyGridItems();
         this.var_233 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_1811) as IWindowContainer;
            ...
            _loc3_.width = this.var_642.width;
            _loc3_.height = this.var_642.height;
            this.var_543.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_642.width,this.var_642.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_642,this.var_642.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1196.width,this.var_1196.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_972.width,this.var_972.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_972,this.var_972.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_543.getGridItemIndex(param1.target as IWindow);
