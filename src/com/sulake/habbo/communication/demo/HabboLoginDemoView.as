package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_490:String = "InitConnection";
       
      
      private var var_182:HabboCommunicationDemo;
      ...
      {
         this.var_182.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_182 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_182.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_149,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_182.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2039,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_182.habboConfiguration)
            {
               if(this.var_182.habboConfiguration.getKey("use.sso","PresentHandler") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_182.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_182.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2039,"/");
         ...
         {
            dispatchEvent(new Event(const_490));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_182.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_157.removeEventListener(SSOTicket.const_412,this.onParsedTicketSuccess);
            this.var_157.removeEventListener(SSOTicket.const_368,this.onParsedTicketFailure);
            this.var_157 = null;
         }
         var _loc2_:String = "";
         if(this.var_182.habboConfiguration != null)
         {
            _loc2_ = this.var_182.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_182.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_157 = new SSOTicket(this.var_182.assets,_loc3_,_loc2_,param1);
         this.var_157.addEventListener(SSOTicket.const_412,this.onParsedTicketSuccess);
         this.var_157.addEventListener(SSOTicket.const_368,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_182.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_182.flashClientUrl = this.var_157.flashClientUrl;
         this.var_182.ssoTicket = this.var_157.ticket;
         this.var_157.removeEventListener(SSOTicket.const_412,this.onParsedTicketSuccess);
         this.var_157.removeEventListener(SSOTicket.const_368,this.onParsedTicketFailure);
         this.var_157 = null;
         dispatchEvent(new Event(const_490));
