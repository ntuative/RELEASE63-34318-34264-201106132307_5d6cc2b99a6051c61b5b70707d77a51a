package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_134:GuestRoomData;
      
      private var var_2776:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2776 = new RoomThumbnailRenderer(param1);
         this.var_954 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_203 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_203.visible = true;
         Util.hideChildren(this.var_203);
         this.refreshRoomName(this.var_203,this.var_134);
         if(this.var_134.ownerName != "" && this.var_134.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_203,!!this.var_134.event ? this.getEventCtg(this.var_134.categoryId) : this.getRoomCtg(this.var_134.categoryId));
         this.refreshRoomDesc(this.var_203,this.var_134);
         this.refreshExtraCont();
         this.refreshInfo(this.var_203,"trading_allowed",this.var_134.allowTrading);
         this.refreshInfo(this.var_203,"doormode_doorbell",this.var_134.doorMode == 1);
         this.refreshInfo(this.var_203,"doormode_password",this.var_134.doorMode == 2);
         Util.moveChildrenToColumn(this.var_203,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_203.height = Util.getLowestPoint(this.var_203);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_203,this.var_134.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_203.findChildByName("roomowner"));
         if(this.var_134.ownerName != "" && this.var_134.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_203.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_954.refreshTags(_loc1_,this.var_134.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_134.score);
         if(this.var_134.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_134.eventCreationTime);
         }
         this.var_2776.refreshThumbnail(_loc1_,this.var_134.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_214,Tabs.const_241,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
