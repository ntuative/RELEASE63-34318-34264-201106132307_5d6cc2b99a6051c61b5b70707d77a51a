package com.sulake.habbo.room.utils
{
   import com.sulake.core.utils.Map;
   
   public class RoomInstanceData
   {
       
      
      private var _roomId:int = 0;
      
      private var _roomCategory:int = 0;
      
      private var var_788:TileHeightMap = null;
      
      private var var_1179:LegacyWallGeometry = null;
      
      private var var_1178:RoomCamera = null;
      
      private var var_789:SelectedRoomObjectData = null;
      
      private var var_790:SelectedRoomObjectData = null;
      
      private var _worldType:String = null;
      
      private var var_537:Map;
      
      private var var_538:Map;
      
      public function RoomInstanceData(param1:int, param2:int)
      {
         this.var_537 = new Map();
         this.var_538 = new Map();
         super();
         this._roomId = param1;
         this._roomCategory = param2;
         this.var_1179 = new LegacyWallGeometry();
         this.var_1178 = new RoomCamera();
      }
      
      public function get roomId() : int
      {
         return this._roomId;
      }
      
      public function get roomCategory() : int
      {
         return this._roomCategory;
      }
      
      public function get tileHeightMap() : TileHeightMap
      {
         return this.var_788;
      }
      
      public function set tileHeightMap(param1:TileHeightMap) : void
      {
         if(this.var_788 != null)
         {
            this.var_788.dispose();
         }
         this.var_788 = param1;
      }
      
      public function get legacyGeometry() : LegacyWallGeometry
      {
         return this.var_1179;
      }
      
      public function get roomCamera() : RoomCamera
      {
         return this.var_1178;
      }
      
      public function get worldType() : String
      {
         return this._worldType;
      }
      
      public function set worldType(param1:String) : void
      {
         this._worldType = param1;
      }
      
      public function get selectedObject() : SelectedRoomObjectData
      {
         return this.var_789;
      }
      
      public function set selectedObject(param1:SelectedRoomObjectData) : void
      {
         if(this.var_789 != null)
         {
            this.var_789.dispose();
         }
         this.var_789 = param1;
      }
      
      public function get placedObject() : SelectedRoomObjectData
      {
         return this.var_790;
      }
      
      public function set placedObject(param1:SelectedRoomObjectData) : void
      {
         if(this.var_790 != null)
         {
            this.var_790.dispose();
         }
         this.var_790 = param1;
      }
      
      public function dispose() : void
      {
         if(this.var_788 != null)
         {
            this.var_788.dispose();
            this.var_788 = null;
         }
         if(this.var_1179 != null)
         {
            this.var_1179.dispose();
            this.var_1179 = null;
         }
         if(this.var_1178 != null)
         {
            this.var_1178.dispose();
            this.var_1178 = null;
         }
         if(this.var_789 != null)
         {
            this.var_789.dispose();
            this.var_789 = null;
         }
         if(this.var_790 != null)
         {
            this.var_790.dispose();
            this.var_790 = null;
         }
         if(this.var_537 != null)
         {
            this.var_537.dispose();
            this.var_537 = null;
         }
         if(this.var_538 != null)
         {
            this.var_538.dispose();
            this.var_538 = null;
         }
      }
      
      public function addFurnitureData(param1:FurnitureData) : void
      {
         if(param1 != null)
         {
            this.var_537.remove(param1.id);
            this.var_537.add(param1.id,param1);
         }
      }
      
      public function getFurnitureData() : FurnitureData
      {
         if(this.var_537.length > 0)
         {
            return this.getFurnitureDataWithId(this.var_537.getKey(0));
         }
         return null;
      }
      
      public function getFurnitureDataWithId(param1:int) : FurnitureData
      {
         return this.var_537.remove(param1);
      }
      
      public function addWallItemData(param1:FurnitureData) : void
      {
         if(param1 != null)
         {
            this.var_538.remove(param1.id);
            this.var_538.add(param1.id,param1);
         }
      }
      
      public function getWallItemData() : FurnitureData
      {
         if(this.var_538.length > 0)
         {
            return this.getWallItemDataWithId(this.var_538.getKey(0));
         }
         return null;
      }
      
      public function getWallItemDataWithId(param1:int) : FurnitureData
      {
         return this.var_538.remove(param1);
      }
   }
}
