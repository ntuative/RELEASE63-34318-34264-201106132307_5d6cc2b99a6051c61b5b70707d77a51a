package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.var_307.length)
         {
            if(this.var_307[_loc3_].url == param1.url)
            {
               return this.var_307[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         this.removeLibraryLoader(_loc2_);
         this.error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1697);
      }
      ...
      protected function removeLibraryLoader(param1:LibraryLoader) : void
      {
         ...
         {
            if(this.var_307[_loc2_] == param1)
            {
               this.var_307.splice(_loc2_,1);
               break;
            }
            _loc2_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_838);
                  }
                  k++;
               }
            }
            componentString = item.attribute("Chase");
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_838);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_838);
                  }
               }
      ...
      public final function attachComponent(param1:Component, param2:Array) : void
      {
         ...
         if(param1.locked)
         {
            param1.events.addEventListener(const_460,this.lockEventHandler);
         }
      ...
      public final function detachComponent(param1:Component) : void
      {
         ...
         while(_loc5_ < this._components.length)
         {
            if(this._components[_loc5_] == param1)
            {
               ...
               param1.events.removeEventListener(const_460,this.lockEventHandler);
               return;
            }
            _loc5_++;
         }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               this.error("Interface " + param1 + " still unavailabe!",true,Core.const_1902);
            }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_307 != null)
            {
               _loc1_ = uint(this.var_307.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_307[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_307 = null;
      ...
      private function lockEventHandler(param1:LockEvent) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:* = null;
         if(param1.type == const_460)
         {
            _loc2_ = param1.unknown as Component;
            _loc2_.events.removeEventListener(const_460,this.lockEventHandler);
            if(!disposed)
            {
               _loc3_ = new Array();
