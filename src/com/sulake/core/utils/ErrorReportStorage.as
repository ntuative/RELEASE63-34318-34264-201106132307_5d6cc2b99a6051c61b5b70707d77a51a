package com.sulake.core.utils
{
   public class ErrorReportStorage
   {
      ...
      private static var var_1306:Map = new Map();
      
      private static var var_866:Map = new Map();
      ...
      public static function getDebugData() : String
      {
         var _loc1_:String = "";
         var _loc2_:int = 0;
         while(_loc2_ < var_866.length)
         {
            if(_loc2_ == 0)
            {
               _loc1_ = var_866.getWithIndex(_loc2_);
            }
            else
            {
               _loc1_ = _loc1_ + " ** " + var_866.getWithIndex(_loc2_);
            }
            _loc2_++;
         }
