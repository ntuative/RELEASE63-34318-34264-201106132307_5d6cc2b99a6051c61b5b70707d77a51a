package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected static var var_427:Point = new Point();
       
      
      protected var var_734:Point;
      ...
      protected var var_912:IWindowContextStateListener;
      ...
      {
         super();
         this.var_734 = new Point();
         if(var_355 == null)
         {
            var_355 = new Array();
            var_355[0] = MouseCursorType.const_341;
            var_355[1] = MouseCursorType.const_31;
            var_355[2] = MouseCursorType.const_341;
            var_355[3] = MouseCursorType.const_341;
            var_355[4] = MouseCursorType.const_341;
            var_355[5] = MouseCursorType.const_31;
            var_355[6] = MouseCursorType.const_341;
         }
         if(var_309 == null)
         {
            var_309 = new Array();
            var_309[0] = WindowState.const_87;
            var_309[1] = WindowState.const_92;
            var_309[2] = WindowState.const_91;
            var_309[3] = WindowState.const_70;
            var_309[4] = WindowState.const_55;
            var_309[5] = WindowState.const_99;
            var_309[6] = WindowState.const_88;
         }
      }
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_188 = state.var_1384 as WindowController;
         this.var_170 = state.renderer;
         this.var_912 = state.var_1383;
         eventQueue.begin();
         this.var_734.x = -1;
         this.var_734.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_734.x || event.stageY != this.var_734.y)
            {
               this.var_734.x = event.stageX;
               this.var_734.y = event.stageY;
               array = new Array();
               this.var_94.groupParameterFilteredChildrenUnderPoint(this.var_734,array,WindowParam.const_30);
            }
            ...
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  ...
                  {
                     this.var_71.getGlobalPosition(var_427);
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_71)
                     {
                        ...
                        if(!child.disposed)
                        {
                           child.getGlobalPosition(var_427);
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_71 && !this.var_71.disposed)
                     {
                        ...
                        {
                           this.var_912.mouseEventReceived(event.type,this.var_71);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.var_1384 = this.var_188;
         state.renderer = this.var_170;
         state.var_1383 = this.var_912;
      }
